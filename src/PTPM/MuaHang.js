...
export default class MuaHang extends Component {
    constructor(props) {
        super(props);
    ...
    listenForItems(itemRef) {
        ...
        this.itemRef.ref('User/' + uid).child('Order').on('child_added', (dataSnapshot) => {
            item.push({
                ...
                <View style={{ flex: 1 / 11, backgroundColor: '#55acee' }}>
                    ...
                    <View style={{ flexDirection: 'row', height: 80, marginTop: 5, justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <View style={{ flex: 6 / 8, justifyContent: 'center', alignItems: 'center' }}>
                            <Text style={{ fontSize: 30, color: 'white', height: 50 }}>LICH SU MUA HANG</Text>
                        </View>
                        ...
                        renderItem={({ item }) =>
                            <View style={{ flexDirection: 'column', marginTop: 20, backgroundColor: 'white' }}>
                                <View style={{ backgroundColor: 'white', flexDirection: 'row', borderBottomColor: 'grey', borderBottomWidth: 1 }} >
                                    <TouchableOpacity style={{ marginTop: 10, marginBottom: 5, flexDirection: 'row', }} onPress={() => { [this.props.navigation.navigate('Shop', { uid: item.uid, name: item.nameshop })] }}>

                                        <View style={{ marginLeft: 20, }}>
                                            <Image source={{ uri: item.avatashop }} style={{ width: 40, height: 40, borderRadius:20 }} />
                                        </View>
                                        <View style={{ marginLeft: 10, flexDirection: 'column', alignItems: 'center' }}>
                                            <Text style={{ color: 'black', fontSize: 25, fontWeight: 'cover', marginTop: 5, marginRight: 5 }}>{item.nameshop}</Text>
                                        </View>
                                    </TouchableOpacity>
                                </View>
                                ...
                                <View style={{ flexDirection: 'row', borderBottomColor: 'grey', borderBottomWidth: 1 }}>
                                    ...
                                    <View style={{ flexDirection: 'column', width: width - 130 }}>
                                        <View style={{ flexDirection: 'column', marginTop: 5, marginLeft: 10 }}>
                                            <Text style={{ fontSize: 25, color: 'black', marginTop: 5 }}>Tên sản phẩm: {item.TenSP} </Text>
                                            <View style={{ marginTop: 5, flexDirection:'row' }}>
                                                <Text style={{ color: '#808080', fontSize: 25,  }}>Giá tiền:</Text>
                                                <Text style={{ color: '#ef4056', fontSize: 25, marginLeft:5 }}> {item.GiaSP} VNĐ</Text>
                                            </View>
                                        </View>
